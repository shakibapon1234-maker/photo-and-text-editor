...
const stickerSplitControls = document.getElementById('stickerSplitControls');
const stickerSplitGlassTextInput = document.getElementById('stickerSplitGlassTextInput');
const stickerSplitSolidTextInput = document.getElementById('stickerSplitSolidTextInput');
const stickerSplitGlassTextColor = document.getElementById('stickerSplitGlassTextColor');
const stickerSplitSolidTextColor = document.getElementById('stickerSplitSolidTextColor');
const stickerSplitGlassColor = document.getElementById('stickerSplitGlassColor');
const stickerSplitSolidColor = document.getElementById('stickerSplitSolidColor');
const stickerSplitOrientation = document.getElementById('stickerSplitOrientation');
...
const cubeThemeGrid = document.getElementById('cubeThemeGrid');
...
function updateShadowFrustum() {
  ...
  for (const l of lights.children) {
    if (l.isDirectionalLight && l.shadow) {
      // Keep the shadow camera centred on the moved object. Otherwise a
      // dragged mesh can sit outside the map and only cast a thin sliver.
...
function syncStudioText(newText) {
  const val = newText !== undefined ? newText : getActiveStudioText();
  state.text = val;
...
function drawGlassSolidSplitTexture() {
  ...
  ctx.lineWidth = 8;
  ctx.beginPath();
  if (vertical) { ctx.moveTo(split, 18); ctx.lineTo(split, canvasH - 18); }
  else { ctx.moveTo(18, split); ctx.lineTo(canvasW - 18, split); }
  ctx.stroke();
  ctx.restore();
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.font = `700 ${fit(glassText, vertical ? canvasW * .42 : canvasW - pad * 2)}px ${CANVAS_TEXT_FONT_STACK}`;
  ctx.fillStyle = state.splitGlassTextColor || '#1f2937';
  ctx.strokeStyle = 'rgba(0,0,0,.3)'; ctx.lineWidth = 8;
  ctx.strokeText(glassText, vertical ? split * .5 : canvasW / 2, vertical ? canvasH / 2 : canvasH * .25);
  ctx.fillText(glassText, vertical ? split * .5 : canvasW / 2, vertical ? canvasH / 2 : canvasH * .25);
  ctx.font = `700 ${fit(solidText, vertical ? canvasW * .42 : canvasW - pad * 2)}px ${CANVAS_TEXT_FONT_STACK}`;
  ctx.fillStyle = state.splitSolidTextColor || '#ffffff';
  ctx.strokeText(solidText, vertical ? split + (canvasW - split) / 2 : canvasW / 2, vertical ? canvasH / 2 : canvasH * .74);
  ctx.fillText(solidText, vertical ? split + (canvasW - split) / 2 : canvasW / 2, vertical ? canvasH / 2 : canvasH * .74);
  return { canvas, aspect: canvasW / canvasH };
}
...
function drawStickerCanvasTexture(
  ...
  if (shape === 'paintSplash') {
    ...
    // 0. Outer User Border (if borderWidth > 0 and borderColor set)
    if (borderWidth > 0) {
      ctx.save();
...
function buildStickerCardMesh(text, shape, bgColor, textColor, curveOpts, borderOpts) {
  ...
  if (is3D) {
    ...
    if (font) {
      ...
      // Build the 3D badge text material for every colour mode. Previously
      // only gradient read the main colour-mode state; solid always fell back
      ...
      lines3D.forEach((lineStr, idx) => {
        ...
        if (hasCurvedOrSpacing && hasText) {
          ...
          chars.forEach((ch, ci) => {
            ...
            const charMat = multicolorPalette
              ? buildMaterial(state.materialType, multicolorPalette[letterColorIndex % multicolorPalette.length])
              : mat3D;
            letterColorIndex++;
      ...
      if (usesIndividualLetterTiles) {
        ...
        // drawRedTilesBackground(), so every extruded glyph stands on (or is
        // mounted to) its own corresponding tile instead of floating as a
        // separate word above the row.
        const tileLetters = Array.from(textStr).filter((ch) => ch.trim().length > 0);
        const count = tileLetters.length;
        const canvasW = 1024;
        const canvasH = canvasW / (worldWidth / worldHeight);
        const isWooden = shape === 'woodenBlocks';
        const gap = isWooden ? Math.max(6, canvasW * 0.015) : Math.max(8, canvasW * 0.018);
        const availableWidth = canvasW * (isWooden ? 0.88 : 0.90);
        ...
        text3DGroup.children.forEach((letter, index) => {
          ...
          letter.position.x = (tileCenterX / canvasW - 0.5) * worldWidth;
          letter.position.y = 0;
          letter.rotation.z = 0;
        });
      }
      ...
      if (state.stickerMode === 'standing') {
        ...
        // Rest the glyphs on the tile/pedestal surface instead of centring
        // their bounding box above it, which left a visible floating gap.
...
if (cubeColorPicker) cubeColorPicker.addEventListener('input', () => { state.cubeColor = cubeColorPicker.value; state.cubeTheme = 'classic'; if (cubeThemeGrid) setActivePreset(cubeThemeGrid, 'cubeTheme', state.cubeTheme); if (state.contentMode === 'cube') scheduleRebuild(); });
if (cubeTextColorPicker) cubeTextColorPicker.addEventListener('input', () => { state.cubeTextColor = cubeTextColorPicker.value; state.cubeTheme = 'classic'; if (cubeThemeGrid) setActivePreset(cubeThemeGrid, 'cubeTheme', state.cubeTheme); if (state.contentMode === 'cube') scheduleRebuild(); });
if (cubeTextBorderPicker) cubeTextBorderPicker.addEventListener('input', () => { state.cubeTextBorder = cubeTextBorderPicker.value; state.cubeTheme = 'classic'; if (cubeThemeGrid) setActivePreset(cubeThemeGrid, 'cubeTheme', state.cubeTheme); if (state.contentMode === 'cube') scheduleRebuild(); });
...
colorPicker.addEventListener('input', () => {
  ...
  // A standing/wall badge has a background card plus a separate group of
